...
-- The unnest sections of the query enables one to be able to have a tidy table that has relevant columns that can be used to different types of visualizations
...
----Gets the labels of the admin levels and no.of questions per category
select
ha.id,
max(hcl.assessment_date) as assessment_date,
...
hcl.modified_at,
hcl.enumerator 
from hc_assessment ha 
left join csv.hc_assessment_questions_per_category haqpc on ha.indicators_category=haqpc.indicators_category ----The hc_assessment_questions_per_category table has the list of indicator categories with no.of questions per category
left join templates.health_center_level_monitoring_and_assessment_of_readiness hcl on ha.id=hcl.id --- Gets the vaccine label from the labels table
